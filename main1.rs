...
struct Word {
    ...
    median_ngram: usize, // Median n-gram order for this word (for coloring)
    render_score: f32, // Combined score for rendering priority (freq + ngram diversity)
    should_render: bool, // Top 25% or activated by query
}
...
struct NgramTrieNode {
    // Immediate next words and their counts
    continuations: HashMap<String, usize>,
    // Next level of the trie
...
struct WordMap {
    ...
    // OPTIMIZED: Single trie instead of 6 hashmaps!
    ...
    // Target chunk size for file splitting
...
impl WordMap {
    fn new() -> Self {
        WordMap {
            ...
            ngram_trie: NgramTrie::new(7),  // Start with max order 7
    ...
    fn load_file(&mut self, path: &str) {
        ...
        println!("[WEIGHT] File: {} tokens {} chunks of ~{} tokens", 
        ...
        // Create spiral visualization for NEW words only
        ...
        let all_words: Vec<String> = sorted_words.iter()
            .map(|(w, _)| (*w).clone())
            .collect();
        ...
        if !all_words.is_empty() {
            ...
            for (i, word) in all_words.iter().enumerate() {
                let total_index = start_index + i;
                let theta = (total_index as f32) * 2.0;
                let r = 20.0 + (total_index as f32) * 0.25;
                
                self.words.push(Word {
                    text: word.clone(),
                    x: theta.cos() * r,
                    y: theta.sin() * r,
                    freq: *new_freq.get(word).unwrap_or(&1),
                    base_size: 3.0,  
                    activation: 0.0,
                    layer: self.files_loaded - 1,
                    in_context: false,
                    embedding: None,
                    median_ngram: 3, // Will be computed later
                    render_score: 0.0, // Will be computed later
                    should_render: false, // Will be set after computing scores
                });
            }
        }
        
        println!("[DONE] Total words: {} | Files loaded: {}", 
            self.words.len(), self.files_loaded);
        ...
        // Compute median n-grams for coloring
        self.compute_word_median_ngrams();
    ...
    // ========================================================================
    // LAYER 3: GENERATION ENGINE (Using optimized trie)
    // ========================================================================
    ...
    fn get_ngram_color(ngram_order: usize) -> Color {
        match ngram_order {
            2 => Color::new(0.0, 1.0, 0.0, 1.0),      // Green
            3 => Color::new(0.0, 1.0, 1.0, 1.0),      // Cyan
            4 => Color::new(1.0, 1.0, 0.0, 1.0),      // Yellow
            5 => Color::new(1.0, 0.6, 0.0, 1.0),      // Orange
            6 => Color::new(1.0, 0.0, 1.0, 1.0),      // Magenta
            7 => Color::new(1.0, 0.2, 0.2, 1.0),      // Red
        ...
        for i in 0..(self.words.len() - 1) {
            ...
            if let Some(best) = scored.first() {
                if best.coherence > 0.89 {
                    ...
                    let center_angle = mid_y.atan2(mid_x);
                    let offset_distance = 15.0;
                    let bridge_x = mid_x + center_angle.cos() * offset_distance;
                    let bridge_y = mid_y + center_angle.sin() * offset_distance;
    ...
    fn compute_word_median_ngrams(&mut self) {
        println!("\n[NGRAM] Computing median n-gram orders and diversity for all words...");
        
        for word in &mut self.words {
            let context = vec![word.text.clone()];
            let candidates = self.ngram_trie.get_candidates(&context, 100);
            
            if candidates.is_empty() {
                word.median_ngram = 3; // Default fallback
                word.render_score = word.freq as f32; // Just use frequency
                continue;
            }
            
            // Collect all ngram orders weighted by their frequency
            let mut ngram_orders = Vec::new();
            let mut ngram_counts: HashMap<usize, usize> = HashMap::new();
            
            for (_word_text, _score, order, count) in &candidates {
                *ngram_counts.entry(*order).or_insert(0) += count;
                for _ in 0..*count {
                    ngram_orders.push(*order);
                }
            }
            
            if ngram_orders.is_empty() {
                word.median_ngram = 3;
                word.render_score = word.freq as f32;
                continue;
            }
            
            // Sort and find median
            ngram_orders.sort_unstable();
            let median_idx = ngram_orders.len() / 2;
            word.median_ngram = ngram_orders[median_idx];
            
            // Calculate diversity: entropy of n-gram order distribution
            // Higher diversity = word appears in many different n-gram contexts
            ...
            // Normalize entropy to [0, 1] range (max entropy for 7 orders â‰ˆ 2.807)
            let diversity = (entropy / 2.807).min(1.0);
            
            // Calculate render score: freq * diversity
            // High frequency + high diversity = most important to render
            word.render_score = word.freq as f32 * (diversity + 0.1); // Add 0.1 to avoid zero
        }
        
        // Determine top 25% threshold
        let mut scores: Vec<f32> = self.words.iter().map(|w| w.render_score).collect();
        scores.sort_by(|a, b| b.partial_cmp(a).unwrap());
        
        let threshold_idx = (scores.len() as f32 * 0.25) as usize;
        let threshold = if threshold_idx < scores.len() {
            scores[threshold_idx]
        } else {
            0.0
        };
        
        // Mark top 25% for rendering
        let mut render_count = 0;
        for word in &mut self.words {
            word.should_render = word.render_score >= threshold;
            if word.should_render {
                render_count += 1;
            }
        }
        
        println!("[NGRAM] Computed scores for {} words", self.words.len());
        println!("[NGRAM] Top 25% threshold: {:.2}", threshold);
        println!("[NGRAM] Will render {} / {} words by default", render_count, self.words.len());
    }
    ...
    fn update_activations(&mut self) {
        for word in &mut self.words {
            word.activation *= 0.95;
        }
        
        if let Some(selected) = &self.selected {
            if let Some(word) = self.words.iter_mut().find(|w| &w.text == selected) {
                word.activation = 1.0;
                word.should_render = true; // Ensure selected word is rendered
            }
            
            // Get related words from trie
            ...
            if !related.is_empty() {
                ...
                // Collect activation updates first to avoid borrow conflict
                ...
                // Apply activation updates and mark for rendering
                for (word_text, new_activation) in activation_updates {
                    if let Some(word) = self.words.iter_mut().find(|w| w.text == word_text) {
                        ...
                        if word.activation > 0.05 {
                            word.should_render = true; // Activated words should render
                        }
                    }
                }
            }
        ...
        if let Some(selected) = &self.selected {
            if let Some(selected_word) = self.words.iter().find(|w| &w.text == selected) {
                ...
                for (i, candidate) in scored.iter().take(8).enumerate() {
                    ...
                    let offset = 80.0 + (candidate.ngram_order - 2) as f32 * 15.0;
                    
                    self.expanded_words.push(ExpandedWord {
                        text: candidate.word.clone(),
                        x: selected_word.x + angle.cos() * offset,
                        y: selected_word.y + angle.sin() * offset,
    ...
    fn screen_to_world(&self, x: f32, y: f32) -> (f32, f32) {
        let world_x = (x - screen_width() / 2.0 - self.camera_x) / self.camera_zoom;
        let world_y = (y - screen_height() / 2.0 - self.camera_y) / self.camera_zoom;
        (world_x, world_y)
    }
    ...
    fn find_word_at(&self, x: f32, y: f32) -> Option<String> {
        ...
        for word in &self.expanded_words {
            ...
            if dx*dx + dy*dy < (word.size * 2.0) * (word.size * 2.0) {
        ...
        for word in &self.words {
            ...
            let visual_size = word.base_size * (1.0 + word.activation * 3.0) * size_mult;
            
            let click_radius = if word.activation > 0.05 {
                visual_size * 2.0
            } else {
                word.base_size * 2.0
            };
    ...
    fn handle_input(&mut self) {
        ...
        if is_key_pressed(KeyCode::N) && !self.text_input_active && !self.generating {
            ...
            if self.config_mode {
                ...
                if !self.config_input.is_empty() {
                    if let Ok(value) = self.config_input.parse::<usize>() {
                        if value >= 2 && value <= 12 {  // Support up to 12-grams!
        ...
        if self.generation_timer >= self.generation_delay {
            ...
            if self.playback_index < self.best_path.len() {
                ...
                if let Some(word) = self.words.iter_mut().find(|w| &w.text == current_word) {
                    word.activation = 1.0;
                    word.should_render = true; // Ensure generated word is rendered
                }
    ...
    fn draw(&self) {
        ...
        // OPTIMIZED: Culling with proper margins
        let margin = 100.0 * self.camera_zoom;
        
        // Draw connection lines FIRST
        if let Some(selected) = &self.selected {
            if let Some(selected_word) = self.words.iter().find(|w| &w.text == selected) {
                let sel_x = center_x + selected_word.x * self.camera_zoom;
                let sel_y = center_y + selected_word.y * self.camera_zoom;
                
                // Lines to expanded words
                for word in &self.expanded_words {
                    let screen_x = center_x + word.x * self.camera_zoom;
                    let screen_y = center_y + word.y * self.camera_zoom;
                    
                    let line_color = match word.ngram_order {
                        2 => Color::new(0.0, 1.0, 0.0, 0.4),
                        3 => Color::new(0.0, 1.0, 1.0, 0.4),
                        4 => Color::new(1.0, 1.0, 0.0, 0.4),
                        5 => Color::new(1.0, 0.6, 0.0, 0.4),
                        6 => Color::new(1.0, 0.0, 1.0, 0.4),
                        7 => Color::new(1.0, 0.2, 0.2, 0.4),
                        _ => Color::new(1.0, 1.0, 1.0, 0.4),
                    };
                    
                    draw_line(sel_x, sel_y, screen_x, screen_y, 1.5, line_color);
                }
            }
        }
        
        // Draw base word dots with NGRAM COLORING and TOP 25% FILTER
        for word in &self.words {
            // Skip if not in top 25% and not activated/selected
            if !word.should_render {
                continue;
            }
            ...
            // OPTIMIZED: Skip offscreen words
            if screen_x < -margin || screen_x > screen_width() + margin ||
               screen_y < -margin || screen_y > screen_height() + margin {
                continue;
            }
            
            // Get color based on median n-gram order
            let base_color = Self::get_ngram_color(word.median_ngram);
            
            // Size calculation with activation boost
            let size_mult = if word.in_context { 2.0 } else { 1.0 };
            let activation_boost = if word.activation > 0.05 { 
                1.0 + word.activation * 3.0 
            } else { 
                1.0 
            };
            let size = word.base_size * activation_boost * self.camera_zoom * size_mult;
            
            // Draw glow for activated words
            if word.activation > 0.05 {
                let glow_alpha = word.activation * 0.3;
                draw_circle(screen_x, screen_y, size * 2.0,
                    Color::new(base_color.r, base_color.g, base_color.b, glow_alpha));
            }
            
            // Determine final color based on state
            let final_color = if Some(&word.text) == self.selected.as_ref() {
                CYAN  // Selected word is cyan
            } else if word.in_context {
                YELLOW  // Context words are yellow
            } else if word.activation > 0.3 {
                // Highly activated: blend toward white
                let blend = word.activation;
                Color::new(
                    base_color.r + (1.0 - base_color.r) * blend * 0.5,
                    base_color.g + (1.0 - base_color.g) * blend * 0.5,
                    base_color.b + (1.0 - base_color.b) * blend * 0.5,
                    1.0
                )
            } else {
                // Base ngram color with slight dimming for inactive words
                let dim = if word.activation < 0.05 { 0.6 } else { 0.8 + word.activation * 0.2 };
                Color::new(base_color.r * dim, base_color.g * dim, base_color.b * dim, 1.0)
            };
            
            // Draw the node
            draw_circle(screen_x, screen_y, size, final_color);
            
            // Draw text for activated or selected words
            if word.activation > 0.3 || Some(&word.text) == self.selected.as_ref() {
                let font_size = (size * 1.2).max(12.0);
                draw_text(&word.text, screen_x - size, screen_y + size / 2.0, font_size, WHITE);
            }
        }
        
        // Draw bridge words
        for bridge in &self.bridge_words {
            let screen_x = center_x + bridge.x * self.camera_zoom;
            let screen_y = center_y + bridge.y * self.camera_zoom;
            let size = 4.0 * self.camera_zoom;
            
            let ngram_color = match bridge.ngram_order {
            ...
            let coherence_strength = (bridge.coherence - 0.6) * 2.5;
            let final_color = Color::new(
                ngram_color.r * (1.0 - coherence_strength * 0.2),
                ngram_color.g * (1.0 - coherence_strength * 0.2),
                (ngram_color.b + coherence_strength * 0.4).min(1.0),
                0.8,
            );
            
            draw_circle(screen_x, screen_y, size * 1.5, 
                Color::new(final_color.r, final_color.g, final_color.b, 0.3));
            draw_circle(screen_x, screen_y, size, final_color);
            
            let label = format!("{}[{}]", bridge.text, bridge.ngram_order);
            let font_size = (size * 1.3).max(10.0);
            draw_text(&label, screen_x - size * 0.8, screen_y - size * 1.5, font_size, WHITE);
        }
        ...
        for word in &self.expanded_words {
            let screen_x = center_x + word.x * self.camera_zoom;
            let screen_y = center_y + word.y * self.camera_zoom;
            let size = (word.size * self.camera_zoom).max(4.0);
            ...
            let final_color = if self.use_embeddings && word.coherence > 0.5 {
                let coherence_strength = (word.coherence - 0.5) * 2.0;
                Color::new(
                    ngram_color.r * (1.0 - coherence_strength * 0.3),
                    ngram_color.g * (1.0 - coherence_strength * 0.3),
                    (ngram_color.b + coherence_strength * 0.5).min(1.0),
                    1.0,
                )
            } else {
                ngram_color
            ...
            let font_size = (size * 1.5).max(14.0);
            draw_text(&label, screen_x - size, screen_y + size / 2.0, font_size, WHITE);
        }
        
        // UI
        let mode_text = if self.use_embeddings {
            "MODE: DUAL TOPOLOGY (N-grams + Embeddings)"
        } else {
            "MODE: N-GRAMS ONLY"
        };
        draw_text(mode_text, 10.0, 30.0, 20.0, if self.use_embeddings { CYAN } else { GREEN });
        
        draw_text("Ctrl + (L: Load | E: Embeddings | T: Toggle Embeddings | Q: Query | N: Config NGram Length (2-7))", 
            10.0, 55.0, 18.0, WHITE);
        
        draw_text("ESC: Cancel/Deselect | R: Reset view | C: Clear context | L : Load file | E: Load embeddings", 
            10.0, 75.0, 16.0, Color::new(0.7, 0.7, 0.7, 1.0));
        
        draw_text(&format!("Chunks: {} | Words: {} | Embeddings: {} | Bridges: {} | Order: {}", 
            self.files_loaded, self.words.len(), self.embeddings.embeddings.len(), 
            self.bridge_words.len(), self.max_ngram_order), 
            10.0, 95.0, 16.0, WHITE);
        
        // File list
        let mut file_y = 115.0;
        ...
        for info in sorted_files {
            let display_name = if info.filename.len() > 25 {
                format!("{}...", &info.filename[..22])
            } else {
                info.filename.clone()
            };
            
            let text = format!("  {}G {} {})", 
                info.ngram_level, display_name, info.load_count);
            
            draw_text(&text, 10.0, file_y, 14.0, WHITE);
            file_y += 18.0;
        }
        
        let base_y = 115.0 + (self.loaded_files.len() as f32 * 18.0);
        ...
        if self.config_mode {
            ...
            draw_text(&prompt, 10.0, box_y + 30.0, 24.0, GREEN);
            draw_text("(Enter to submit | ESC or Q to cancel)", 10.0, box_y + 10.0, 16.0, YELLOW);
            ...
            let text = if self.use_embeddings {
                "Ã¢Ëœâ€¦ DUAL TOPOLOGY GENERATION..."
            } else {
                "Ã¢Ëœâ€¦ N-GRAM GENERATION..."
            };
            draw_text(text, 10.0, box_y + 30.0, 20.0, ORANGE);
        } else if self.generating {
            let box_y = screen_height() - 100.0;
            draw_rectangle(0.0, box_y, screen_width(), 100.0, 
                Color::new(0.1, 0.1, 0.1, 0.9));
            
            draw_text("GENERATING...", 10.0, box_y + 20.0, 20.0, CYAN);
            
            let display_text = if self.generated_output.chars().count() > 100 {
                let skip_count = self.generated_output.chars().count().saturating_sub(100);
                format!("...{}", self.generated_output.chars().skip(skip_count).collect::<String>())
            } else {
                self.generated_output.clone()
            };
            draw_text(&display_text, 10.0, box_y + 50.0, 16.0, WHITE);
            draw_text(&format!("Tokens: {}", self.playback_index), 10.0, box_y + 75.0, 14.0, YELLOW);
        } else {
            let hint_y = screen_height() - 30.0;
            draw_text("ESC: Deselect | Q: Query | R: Reset | C: Clear context", 
                10.0, hint_y, 18.0, Color::new(0.5, 0.5, 0.5, 1.0));
        }
    }
}

#[macroquad::main("M2 - OPTIMIZED Dual Topology")]
async fn main() {
    ...
    if word_map.files_loaded == 0 {
        println!("\n========================================");
        println!("  M2 v3.0 - OPTIMIZED TRIE STRUCTURE");
        println!("========================================");
        println!("\n Trie-based n-grams (10-100x faster!)");
        println!(" Support for 2-12 gram orders");
        println!(" Batch loading with automatic weighting");
        println!(" Bigger, clearer nodes");
        println!("\nPress L to load corpus");
        println!("Press N to configure n-gram order (2-12)");
        println!("========================================\n");
    }
