...
struct Word {
    ...
    ngram_confidence: f32,      // Average confidence (0.0-1.0)
    avg_ngram_order: f32,       // NEW: Average n-gram order across all connections
}
...
impl WordMap {
    ...
    // NEW: Compute average n-gram confidence (considers ALL connections, not just max)
    fn compute_ngram_confidence(&mut self) {
        ...
        for word in &mut self.words {
            ...
            if !candidates.is_empty() {
                // Calculate AVERAGE n-gram order across all connections
                let total_order: usize = candidates.iter()
                    .map(|(_, _, order, count)| order * count)  // Weight by frequency
                    .sum();
    ...
    // NEW: Convert confidence (0.0-1.0) to HSV color
    // Low confidence (0.0) = gray/muted
    // High confidence (1.0) = vibrant colors
    fn confidence_to_hsv_color(confidence: f32, position: (f32, f32)) -> Color {
        // Use position to determine hue (creates spatial color variation)
        let angle = position.1.atan2(position.0);
        let hue = ((angle + std::f32::consts::PI) / (2.0 * std::f32::consts::PI)) * 360.0;
        
        // Confidence controls saturation and value
        // Low confidence = desaturated (gray)
        // High confidence = saturated and bright
        let saturation = 0.2 + confidence * 0.7;  // 0.2 to 0.9
        let value = 0.4 + confidence * 0.5;        // 0.4 to 0.9
    ...
    fn generate_dual_topology(&mut self, seed: Vec<String>, max_tokens: usize) -> Vec<String> {
        ...
        for step in 0..max_tokens {
            ...
            let ngram_candidates = self.ngram_trie.get_candidates(&output, 1500000);
    ...
    // Build a quick index for lookup by text
    let index: HashMap<&str, &Word> = self.words.iter().map(|w| (w.text.as_str(), w)).collect();

    // For each word, try a few top n-gram neighbors and pick a coherent bridge
    for w in &self.words {
        let emb1 = if let Some(e) = &w.embedding { e } else { continue };
        // Pull a bigger neighbor set from n-grams
        let cand = self.ngram_trie.get_candidates(&[w.text.clone()], 120);
        if cand.is_empty() { continue; }

        // Try top-K neighbors that also have embeddings
        ...
        for (nbr, _flu, ord, _cnt) in cand.into_iter().take(25) {
            ...
            if let Some(s0) = scored.first() {
                // relax threshold a bit; tune live (0.68–0.78)
        ...
        if let Some((token, coh, ord)) = best {
            // place the bridge between positions in your disc
            let (x, y) = (w.x, w.y);
    ...
    fn draw(&self) {
    // Only clear if something is happening
    if self.generating || self.exploring || self.any_active || self.redraw_needed ||self.config_mode {
        ...
        // Replace the entire semantic cell drawing section in draw() with this:

        if self.semantic_mode {
            // NEW: Draw cells incrementally over multiple frames with fading
            let cells_per_frame = 6000; // Adjust this to control drawing speed
            ...
            for (cell_idx, cell) in self.semantic_cells.iter().enumerate() {
                ...
                // Calculate fade based on how recently this cell was drawn
                // Cells fade out over time unless refreshed
                let frame_age = (self.frame_counter as i32 - (cell_idx % 60) as i32).abs() as f32;
                let fade_factor = (1.0 - (frame_age / 120.0)).max(0.0); // Fade over 45 frames
                
                // Only draw cells that should be visible this frame
                let should_draw_this_frame = cell_idx % 60 == (self.frame_counter as usize % 60);
                
                // OR draw based on activation
                ...
                let boosted_color = Color::new(
                    ...
                    ((cell.color.a + center_activation * 0.1).min(0.3)) * fade_factor, // Apply fade
                );
                ...
                for i in 0..screen_corners.len() {
                    ...
                    let border_alpha = (base_alpha + center_activation * 0.3) * fade_factor; // Apply fade
                    draw_line(
        ...
        draw_text(&format!("Files: {} | Words: {} | Cells: {} | Bridges: {} | Order: {}", 
            self.files_loaded,
            self.words.len(),self.semantic_cells.len(), self.bridge_words.len(), self.max_ngram_order), 
        ...
        if let Some(selected) = &self.selected {
            // NEW: Display confidence info for selected word
