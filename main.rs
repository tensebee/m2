...
struct LruCtxCache<V> {
    cap: usize,
    map: HashMap<String, V>,
    q: VecDeque<String>,
}
impl<V> LruCtxCache<V> {
    fn new(cap: usize) -> Self { Self { cap, map: HashMap::new(), q: VecDeque::new() } }
    fn get(&mut self, k: &str) -> Option<&V> {
        if self.map.contains_key(k) {
            let k_owned = k.to_string();
            if let Some(pos) = self.q.iter().position(|x| x == &k_owned) {
                if pos + 1 != self.q.len() {
                    self.q.remove(pos);
                    self.q.push_back(k_owned);
                }
            }
    ...
    fn put(&mut self, k: String, v: V) {
        if self.map.contains_key(&k) {
            ...
            if let Some(pos) = self.q.iter().position(|x| x == &k) {
                if pos + 1 != self.q.len() {
                    self.q.remove(pos);
                    self.q.push_back(k);
                }
...
struct DiskBatchManager {
    ...
    ctx_cache: RefCell<LruCtxCache<Vec<(String, usize)>>>,
}
...
impl DiskBatchManager {
    fn new(max_batch_size: usize) -> Self {
        ...
        DiskBatchManager {
            ...
            max_batch_size,
            batch_files: Vec::new(),
    ctx_cache: RefCell::new(LruCtxCache::new(250_000)),
}
    }
...
fn get_candidates(&self, context: &str, max_results: usize) -> Vec<(String, usize)> {
    // Try cache first
    {
        ...
        if let Some(v) = cache.get(context) {
            let mut v2 = v.clone();
            v2.sort_by(|a, b| b.1.cmp(&a.1));
            if v2.len() > max_results { v2.truncate(max_results); }
            return v2;
        }
    }
    // Merge RAM + disk
    ...
    if let Some(continuations) = self.current_batch.get(context) {
        for (word, count) in continuations {
            *all.entry(word.clone()).or_insert(0) += *count;
        }
    }
    for batch_file in &self.batch_files {
        if let Ok(file) = fs::File::open(batch_file) {
            ...
            for line in reader.lines().flatten() {
                if !line.starts_with(context) { continue; }
                let mut it = line.splitn(3, '\t');
                let ctx = it.next().unwrap_or("");
                if ctx != context { continue; }
                let w = it.next().unwrap_or("");
                let c = it.next().unwrap_or("0").parse::<usize>().unwrap_or(0);
                if !w.is_empty() && c > 0 {
                    *all.entry(w.to_string()).or_insert(0) += c;
                }
            }
        }
    }
    let mut vecd: Vec<(String, usize)> = all.into_iter().collect();
    vecd.sort_by(|a, b| b.1.cmp(&a.1));
    {
        let mut cache = self.ctx_cache.borrow_mut();
        cache.put(context.to_string(), vecd.clone());
    }
    if vecd.len() > max_results { vecd.truncate(max_results); }
    vecd
}
    ...
    /// Pre-warm the cache for a set of contexts by scanning each batch file ONCE.
    /// `contexts` are the exact keys `get_candidates()` will ask for (e.g., single-word contexts).
    fn prewarm_contexts(&self, contexts: &[String]) {
        if contexts.is_empty() { return; }

        // Build a lookup of needed contexts
        let mut need: HashMap<&str, HashMap<String, usize>> = HashMap::new();
        for c in contexts {
            need.entry(c.as_str()).or_insert_with(HashMap::new);
        }

        // Merge in-memory batch first
        for (ctx, conts) in &self.current_batch {
            if let Some(acc) = need.get_mut(ctx.as_str()) {
                for (w, c) in conts {
                    *acc.entry(w.clone()).or_insert(0) += *c;
                }
            }
        }

        // Scan each disk batch ONCE and only collect lines for contexts we care about
        for batch_file in &self.batch_files {
            if let Ok(file) = fs::File::open(batch_file) {
                let reader = BufReader::new(file);
                for line in reader.lines().flatten() {
                    if let Some(tab_pos) = line.find('\t') {
                        let ctx = &line[..tab_pos];
                        if let Some(acc) = need.get_mut(ctx) {
                            let rest = &line[tab_pos+1..];
                            if let Some(tab2) = rest.find('\t') {
                                let w = &rest[..tab2];
                                let c = rest[tab2+1..].parse::<usize>().unwrap_or(0);
                                if !w.is_empty() && c > 0 {
        ...
        // Commit results into the LRU cache
        let mut cache = self.ctx_cache.borrow_mut();
        for (ctx, acc) in need {
            let mut v: Vec<(String, usize)> = acc.into_iter().collect();
            v.sort_by(|a, b| b.1.cmp(&a.1));
            cache.put(ctx.to_string(), v);
        }
    }
}
...
struct WordMap {
    ...
    // Cached node-mode render
    node_rt: Option<RenderTarget>,
    node_cache_dirty: bool,
    node_cached_words: usize,
    node_cached_camera: (f32, f32, f32),
    node_cached_selected: Option<String>,
    
    // Cached semantic-cells render
    cells_rt: Option<RenderTarget>,
    cells_cache_dirty: bool,
    cells_cached_words: usize,
    cells_cached_camera: (f32, f32, f32),
    }
...
impl WordMap {
    fn ensure_cells_rt(&mut self) {
        let w = screen_width() as u32;
        let h = screen_height() as u32;
        let recreate = match &self.cells_rt {
            None => true,
        ...
        if recreate {
            ...
            self.cells_rt = Some(rt);
            self.cells_cache_dirty = true;
        }
    }

    fn render_cells_layer(&mut self) {
        self.ensure_cells_rt();
        let rt = self.cells_rt.as_ref().unwrap();
        let cam = Camera2D {
        ...
        // Draw semantic cells statically (no per-frame fade/animation)
        for cell in &self.semantic_cells {
            ...
            let screen_corners: Vec<_> = cell.corners.iter()
                ...
                .collect();
            if screen_corners.len() < 3 { continue; }

            let base = Color::new(cell.color.r, cell.color.g, cell.color.b, cell.color.a.min(0.3));
            ...
            for i in 0..screen_corners.len() {
                ...
                draw_line(
                    ...
                    1.0,
                    Color::new(base.r, base.g, base.b, (base.a + 0.05).min(0.6))
                );
            }
        }

        set_default_camera();
        self.cells_cached_words = self.words.len();
        self.cells_cached_camera = (self.camera_x, self.camera_y, self.camera_zoom);
        self.cells_cache_dirty = false;
    }

    fn ensure_node_rt(&mut self) {
        let w = screen_width() as u32;
        let h = screen_height() as u32;
        let recreate = match &self.node_rt {
            None => true,
            Some(rt) => rt.texture.width() as u32 != w || rt.texture.height() as u32 != h,
        };
        if recreate {
            let rt = render_target(w, h);
            rt.texture.set_filter(FilterMode::Linear);
            self.node_rt = Some(rt);
            self.node_cache_dirty = true;
        }
    }

    fn render_node_layer(&mut self) {
        self.ensure_node_rt();
        let rt = self.node_rt.as_ref().unwrap();
        // Render to offscreen target using a camera bound to the RT
        let cam = Camera2D {
            render_target: Some(rt.clone()),
            zoom: vec2(2.0 / screen_width(), -2.0 / screen_height()),
            target: vec2(0.0, 0.0),
        ...
        // Draw the static node layer (boundary circles and nodes/labels)
        let center_x = screen_width()/2.0 + self.camera_x;
        let center_y = screen_height()/2.0 + self.camera_y;
        let boundary_radius = self.poincare.radius * self.camera_zoom;

        // Disc guides
        draw_circle_lines(0.0 + self.camera_x + screen_width()/2.0, 0.0 + self.camera_y + screen_height()/2.0, boundary_radius, 2.0, Color::new(0.3, 0.3, 0.35, 0.6));
        for i in 1..5 {
            let r = (i as f32 / 5.0) * boundary_radius;
            draw_circle_lines(self.camera_x + screen_width()/2.0, self.camera_y + screen_height()/2.0, r, 1.0, Color::new(0.2, 0.2, 0.25, 0.4));
        }

        // Use the same renderable_words logic from draw(), but without per-frame animations
        ...
        if renderable_words.len() > self.render_threshold as usize {
        ...
        for word in renderable_words {
            ...
            let size = base_pixel_size * size_mult;

            let (color, _glow_color) = if Some(&word.text) == self.selected.as_ref() {
                (CYAN, CYAN)
            } else if word.in_context {
                (YELLOW, YELLOW)
            } else {
                let base_color = Self::confidence_to_hsv_color(word.ngram_confidence, (word.x, word.y));
                let blended = Color::new(base_color.r * 0.8, base_color.g * 0.8, base_color.b * 0.8, 0.9);
                (blended, blended)
            };
            ...
            // Labels
            let font_size = (size * 1.3).max(16.0);
            let label_color = Color::new(0.95, 0.95, 0.95, 0.9);
            draw_text(&word.text, screen_x - size, screen_y + size / 2.0, font_size, label_color);
        }

        // Restore default camera back to screen
        set_default_camera();
        self.node_cached_words = self.words.len();
        self.node_cached_camera = (self.camera_x, self.camera_y, self.camera_zoom);
        self.node_cached_selected = self.selected.clone();
    ...
    fn new() -> Self {
        WordMap {
            ...
            node_cache_dirty: true,
        
            node_cached_words: 0,
        
            node_cached_camera: (0.0, 0.0, 1.0),
        
            node_cached_selected: None,
        
            cells_rt: None,
        
            cells_cache_dirty: true,
        
            cells_cached_words: 0,
        
            cells_cached_camera: (0.0, 0.0, 1.0),
        }
    }
    ...
    fn load_file(&mut self, path: &str) {
        ...
        // Prewarm cache for single-token contexts (top by frequency), so confidence is RAM-only
        {
            const MAX_TO_COMPUTE: usize = 20_000;
            let mut idxs: Vec<usize> = (0..self.words.len()).collect();
            idxs.sort_by(|&a, &b| self.words[b].freq.cmp(&self.words[a].freq));
            if idxs.len() > MAX_TO_COMPUTE { idxs.truncate(MAX_TO_COMPUTE); }
            let mut contexts: Vec<String> = Vec::with_capacity(idxs.len());
            for &i in &idxs {
                if !self.words[i].text.is_empty() {
                    contexts.push(self.words[i].text.clone());
                }
            }
            if self.ngram_trie.use_disk {
                println!("[DISK] Prewarming {} confidence contexts...", contexts.len());
                self.ngram_trie.disk_manager.prewarm_contexts(&contexts);
                println!("[DISK] Prewarm complete");
            }
        }
...
fn compute_ngram_confidence(&mut self) {
    const MAX_TO_COMPUTE: usize = 20_000;
    println!("[NGRAM] Computing average n-gram confidence (max {}) ...", MAX_TO_COMPUTE);
    let mut idxs: Vec<usize> = (0..self.words.len()).collect();
    idxs.sort_by(|&a, &b| self.words[b].freq.cmp(&self.words[a].freq));
    if idxs.len() > MAX_TO_COMPUTE { idxs.truncate(MAX_TO_COMPUTE); }
    let mut processed = 0usize;
    for &i in &idxs {
        let context = vec![self.words[i].text.clone()];
        let candidates = self.ngram_trie.get_candidates(&context, 50);
        if candidates.is_empty() {
            self.words[i].avg_ngram_order = self.words[i].avg_ngram_order.max(2.0);
            self.words[i].max_ngram_order = self.words[i].max_ngram_order.max(2);
            self.words[i].ngram_confidence = self.words[i].ngram_confidence.max(0.05);
        } else {
            let total_order: usize = candidates.iter().map(|(_, _, order, count)| order * count).sum();
            let total_count: usize = candidates.iter().map(|(_, _, _, count)| count).sum();
            let avg_order = if total_count > 0 { (total_order as f32) / (total_count as f32) } else { 2.0 };
            let max_order = candidates.iter().map(|(_, _, order, _)| *order).max().unwrap_or(2);
            self.words[i].avg_ngram_order = avg_order;
            self.words[i].max_ngram_order = max_order;
            self.words[i].ngram_confidence = Self::ngram_order_to_confidence(avg_order.round() as usize);
        }
        processed += 1;
        if processed % 5000 == 0 {
            println!("  ...{} / {}", processed, idxs.len());
        }
    }
    println!("[NGRAM] Confidence computed for {} / {} words (bounded).", processed, self.words.len());
}
    ...
    self.node_cache_dirty = true;
    println!("
 Visualization mode: {}", if self.semantic_mode { "SEMANTIC CELLS" } else { "NODE MODE" });
    ...
    fn draw(&mut self) {
        ...
        // Node-mode caching: render once into an offscreen texture, reuse every frame
        if !self.semantic_mode {
            let camera_changed = (self.camera_x - self.node_cached_camera.0).abs() > 1.0
                || (self.camera_y - self.node_cached_camera.1).abs() > 1.0
                || (self.camera_zoom - self.node_cached_camera.2).abs() > 0.01;
            let selection_changed = self.node_cached_selected != self.selected;
            let words_changed = self.node_cached_words != self.words.len();
            if self.node_rt.is_none() || self.node_cache_dirty || camera_changed || selection_changed || words_changed {
                self.render_node_layer();
            }
            // Blit the cached node layer texture to the screen
            if let Some(rt) = &self.node_rt {
                draw_texture(&rt.texture, 0.0, 0.0, WHITE);
            }
            // UI overlays still drawn below will execute; early return would skip overlays, so we don't return here.
        }
        let center_x = screen_width()/2.0 + self.camera_x;
        let center_y = screen_height()/2.0 + self.camera_y;
